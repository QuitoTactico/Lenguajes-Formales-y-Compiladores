def input_recognition(inputs: list[list[str]], debug: bool = False) -> list[dict]:
    ...
    if debug:
        print("automatas_to_read:", automatas_to_read)

    index = 1  # this index is where the "reading pointer" is (row)

    for automata in range(automatas_to_read):
        automata_func_len = int(
            inputs[index][0]
        )  # the first automata input being read. it's the Vlen of the int matrix

        language = inputs[
            index + 1
        ]  # right after that, there is the language characters list
        final_states = list(
            map(int, inputs[index + 2])
        )  # after that, the final states. map is for int formatting
        raw_func = inputs[
            index + 3 : index + 3 + automata_func_len
        ]  # and finally, the func/delta matrix (string format)
...
def function_interpreter(
    raw_func: list[list[str]], language: list[str]
...
def equivalences_recognicer(
    language: list[str],
    final_states: list[int],
    func: dict[tuple[int, str], int],
    debug: bool = False,
) -> list[list[tuple]]:
    ...
    states = set(
        state for state, _ in func.keys()
    )  # the start of each tuple key is a state. set deletes the duplicates
    final_states_set = set(
        final_states
    )  # the same list of final states, but making it a set allows us to...
    ...
    equivs_partitions = [
        final_states_set,
        non_final_states_set,
    ]  # at the end, it will have sets of equivalent states
    work_partition = [
        final_states_set,
        non_final_states_set,
    ]  # we will iterate on this one until it empties
    ...
    while work_partition:  # while the work partition is not empty...
        ...
        for character in language:
            ...
            X = {
                state
                for state in states
                if func.get((state, character)) in extracted_set
            }  # BACKTRACK STATES

            # for each set in the equivs partition (starting with the final states)
            for Y in equivs_partitions[
                :
            ]:  # [:] creates a copy, so we don't worry if the original one is modified

                intersection = (
                    X & Y
                )  # states in both (states that go to the selected set)
                ...
                if intersection and difference:  # if both are not empty...
                    ...
                    equivs_partitions.append(
                        intersection
                    )  # and replace the set with the states that go
    ...
    if debug:
        print("sets of equivalent states (pre-filter):", equivs_partitions)
...
def equivalences_printer(equivalences: list[list[tuple]], debug: bool = False) -> None:
    ...
    sorted_equivalences = sorted(
        equivalences
    )  # i thought i would have to do this manually, lucky me...

    print(
        *sorted_equivalences
    )  # this * is called depacking operator, it's not a pointer
...
if __name__ == "__main__":
    ...
    for n, automata in enumerate(recogniced_automatas):
        ...
        equivalences = equivalences_recognicer(
            language=automata["language"],
