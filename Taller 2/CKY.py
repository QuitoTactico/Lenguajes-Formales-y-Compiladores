...
def input_recognition(inputs: list[list[str]]) -> list[dict]:
    ...
    for _ in range(grammars_to_read):
        ...
        for i in range(index + 1, index + 1 + grammar_len):
            lhs, *rhs = inputs[
                i
            ]  # lhs is the first element, rhs is the rest. rhs are the productions of lhs
        ...
        recogniced_grammars_and_words.append({"grammar": grammar, "words": words})
...
def cky_algorithm(grammar: dict[str, list[str]], word: str, debug: bool) -> bool:
    """Cocke-Kasami-Younger algorithm to validate if a word is generated by a given grammar.
    returns True if the word is generated by the grammar, False otherwise."""

    n = len(word)
    table = [[set() for _ in range(n)] for _ in range(n)]  # set matrix
    ...
    for l in range(2, n + 1):
        for i in range(n - l + 1):  # i = 0, 1, 2, ..., n - l
            j = i + l - 1
            for k in range(i, j):  # k = i, i + 1, ..., j - 1
                for lhs, rhs in grammar.items():
                    for production in rhs:  # production = A -> BC for example
                        if len(production) == 2:
                            B, C = production
                            if (
                                B in table[i][k] and C in table[k + 1][j]
                            ):  # if B and C are in the table
                                table[i][j].add(lhs)  # add A to the table
    ...
    # VALIDATION: if S is in the top right corner of the table, the word is generated by the grammar
...
if __name__ == "__main__":
    ...
    for n, grammar_and_words in enumerate(recogniced_grammars_and_words):
        ...
        for word in grammar_and_words["words"]:
            ...
            validation = cky_algorithm(grammar=grammar, word=word, debug=debug)
